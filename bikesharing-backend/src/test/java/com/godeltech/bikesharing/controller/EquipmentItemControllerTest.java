...
class EquipmentItemControllerTest {
  ...
  public void shouldGetProperResponse() throws Exception {
    ...
    when(service.getAllByStatusCode(CODE_FREE, PAGE_SIZE, PAGE_NUM)).thenReturn(new PageImpl<>(List.of(model)));
    ...
    var actualResponseFromServer = jsonMapper.getResponseFromPage(result, type);

    verify(service).getAllByStatusCode(CODE_FREE, PAGE_SIZE, PAGE_NUM);
